...
public class CustomViewAbove extends ViewGroup {
	...
	//监听手指X与滑动
	private MenuScrollListener mMenuScrollListener;

	public void setmMenuScrollListener(MenuScrollListener mMenuScrollListener) {
		this.mMenuScrollListener = mMenuScrollListener;
	}
	...
	public void setCanResponseView(View view) {
		this.canResponseView = view;
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (mIsBeingDragged) {
				...
				if(mMenuScrollListener != null) {
					mMenuScrollListener.onMenuScroll(leftBound, mLastMotionX);
				}
		...
		case MotionEvent.ACTION_UP:
			if (mIsBeingDragged) {
				...
				// close the menu
				if(canResponseView != null) {
					Rect rect = new Rect();
					canResponseView.getHitRect(rect);

					Log.i("自定义", "behind width=" + getBehindWidth());
					Log.i("自定义", "x=" + ev.getX() + "---y=" + ev.getY());
					Log.i("自定义", "left=" + rect.left + "---right=" + rect.right + "---top=" + rect.top + "---bottom=" + rect.bottom);
					if (rect.contains((int)ev.getX() - getBehindWidth(), (int)ev.getY())) {
						canResponseView.callOnClick();
						Log.i("自定义", "3");
					}
				}
	...
	public interface MenuScrollListener {
		void onMenuScroll(float length, float scroll);
	}
}
